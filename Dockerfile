...
# ComfyUI 5.x passes extra kwargs (attn_mask, transformer_options, etc.) to
# DoubleStreamBlock.forward(). The xflux DSBnew block-level forward only accepts
# (self, img, txt, vec, pe). Fix: replace with **kwargs to absorb all extras.
# Also fix processor-level forwards which have the same gap.
RUN printf 'import os\n\
# Patch 1: block-level forward — add **kwargs to absorb attn_mask, transformer_options, etc.\n\
OLD1="def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor) -> tuple[Tensor, Tensor]:"\n\
NEW1="def forward(self, img: Tensor, txt: Tensor, vec: Tensor, pe: Tensor, **kwargs) -> tuple[Tensor, Tensor]:"\n\
# Patch 2: processor forwards — add attn_mask=None before existing **attention_kwargs\n\
...
for root, dirs, files in os.walk("/comfyui/custom_nodes/"):\n\
    for fname in files:\n\
        ...
        if patched != txt:\n\
            ...
            count = patched.count("**kwargs") + patched.count("attn_mask=None")\n\
